...
bool ULoadingScreenManager::CheckForAnyNeedToShowLoadingScreen()
{
	...
	{
		float LoadingPercent = GetAsyncLoadPercentage(*LoadingMap);
		LoadingProgress = LoadingPercent > 0 ? LoadingPercent / 100.f : 0.f;
	...
	LoadingProgress = FMath::Max(0.7f, LoadingProgress);
	...
	LoadingProgress = FMath::Max(0.9f, LoadingProgress);
	...
	LoadingProgress = FMath::Max(0.95f, LoadingProgress);
	...
	//这里的GameViewportClient有可能为空指针，暂时加一个保护
...
void ULoadingScreenManager::ShowLoadingScreen()
{
	...
	else
	{
		...
		TSubclassOf<UUserWidget> WidgetClass = Settings->LoadingScreenWidget.TryLoadClass<UUserWidget>();
...
void ULoadingScreenManager::SetLoadingScreenContentWidget(TSubclassOf<UUserWidget> NewWidgetClass)
{
