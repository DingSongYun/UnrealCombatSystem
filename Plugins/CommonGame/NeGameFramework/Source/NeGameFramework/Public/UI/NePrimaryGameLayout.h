...
#include "NePrimaryGameLayout.generated.h"
...
class NEGAMEFRAMEWORK_API UNePrimaryGameLayout : public UCommonUserWidget
{
	GENERATED_BODY()

public:
	static UNePrimaryGameLayout* GetPrimaryGameLayoutForPrimaryPlayer(const UObject* WorldContextObject);
	static UNePrimaryGameLayout* GetPrimaryGameLayout(APlayerController* PlayerController);
	static UNePrimaryGameLayout* GetPrimaryGameLayout(ULocalPlayer* LocalPlayer);

public:
	UNePrimaryGameLayout(const FObjectInitializer& ObjectInitializer);
...
public:
	...
	TSharedPtr<FStreamableHandle> PushWidgetToLayerStackAsync(FGameplayTag LayerName, bool bSuspendInputUntilComplete, TSoftClassPtr<UCommonActivatableWidget> ActivatableWidgetClass, TFunction<void(EAsyncWidgetLayerState, ActivatableWidgetT*)> StateFunc)
	{
		...
		const FName SuspendInputToken = NAME_None;// bSuspendInputUntilComplete ? UCommonUIExtensions::SuspendInputForPlayer(GetOwningPlayer(), NAME_PushingWidgetToLayer) : NAME_None;
