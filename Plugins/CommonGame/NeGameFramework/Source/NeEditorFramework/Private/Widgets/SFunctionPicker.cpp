...
void SFunctionPicker::Construct(const FArguments& InArgs, UClass* InFromClass, UFunction* InAllowedSignature)
{
	...
	bIncludeSuperFunction = InArgs._IncludeSuperFunction;
...
TSharedRef<SWidget> SFunctionPicker::OnGenerateDelegateMenu()
{
	...
	MenuBuilder.BeginSection("Functions", LOCTEXT("Functions", "Functions"));
	{
		...
		EFieldIteratorFlags::SuperClassFlags Flag;
		if (bIncludeSuperFunction.Get())
		{
			Flag = EFieldIteratorFlags::IncludeSuper;
		}
		else
		{
			Flag = EFieldIteratorFlags::ExcludeSuper;
		}
		for ( TFieldIterator<UFunction> FuncIt(FromClass, Flag); FuncIt; ++FuncIt )
		{
...
void SFunctionPicker::PickFunction(UFunction* SelectedFunction)
{
	...
	SelectedFunctionDelegate.ExecuteIfBound(FunctionName, SelectedFunction);
}
...
{
	UBlueprint* Blueprint = Cast<UBlueprint>(FromClass->ClassGeneratedBy);
	UFunction* Function = FromClass->FindFunctionByName(CurrentFunction.Get().GetValue());
