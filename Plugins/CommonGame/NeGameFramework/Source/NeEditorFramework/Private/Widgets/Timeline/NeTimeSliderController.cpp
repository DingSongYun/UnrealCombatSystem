...
#define LOCTEXT_NAMESPACE "NeAnimTimeSliderController"
...
struct FNeTimeSliderController::FScrubRangeToScreen
{
	double ViewStart;
...
FNeTimeSliderController::FNeTimeSliderController( const FTimeSliderArgs& InArgs, TWeakPtr<FNeTimelineMode> InTimelineMode, TWeakPtr<SNeTimeline> InWeakTimeline, TSharedPtr<INumericTypeInterface<double>> InSecondaryNumericTypeInterface )
...
FFrameTime FNeTimeSliderController::ComputeFrameTimeFromMouse(const FGeometry& Geometry, FVector2D ScreenSpacePosition, FScrubRangeToScreen RangeToScreen, bool CheckSnapping) const
{
...
FNeTimeSliderController::FScrubPixelRange FNeTimeSliderController::GetHitTestScrubberPixelRange(FFrameTime ScrubTime, const FScrubRangeToScreen& RangeToScreen) const
{
...
FNeTimeSliderController::FScrubPixelRange FNeTimeSliderController::GetScrubberPixelRange(FFrameTime ScrubTime, const FScrubRangeToScreen& RangeToScreen) const
{
	return GetScrubberPixelRange(ScrubTime, GetTickResolution(), GetDisplayRate(), RangeToScreen);
}

FNeTimeSliderController::FScrubPixelRange FNeTimeSliderController::GetScrubberPixelRange(FFrameTime ScrubTime, FFrameRate Resolution, FFrameRate PlayRate, const FScrubRangeToScreen& RangeToScreen, float DilationPixels) const
{
...
struct FNeTimeSliderController::FDrawTickArgs
{
...
void FNeTimeSliderController::DrawTicks( FSlateWindowElementList& OutDrawElements, const TRange<double>& ViewRange, const FScrubRangeToScreen& RangeToScreen, FDrawTickArgs& InArgs ) const
{
	TSharedPtr<SNeTimeline> Timeline = WeakTimeline.Pin();
	if(!Timeline.IsValid())
	{
		return;
	}
...
int32 FNeTimeSliderController::OnPaintTimeSlider( bool bMirrorLabels, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled ) const
{
...
int32 FNeTimeSliderController::DrawSelectionRange(const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FScrubRangeToScreen& RangeToScreen, const FPaintPlaybackRangeArgs& Args) const
{
...
int32 FNeTimeSliderController::DrawPlaybackRange(const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FScrubRangeToScreen& RangeToScreen, const FPaintPlaybackRangeArgs& Args) const
{
...
FReply FNeTimeSliderController::OnMouseButtonDown( SWidget& WidgetOwner, const FGeometry& MyGeometry, const FPointerEvent& MouseEvent )
{
	DistanceDragged = 0;
...
FReply FNeTimeSliderController::OnMouseButtonUp( SWidget& WidgetOwner, const FGeometry& MyGeometry, const FPointerEvent& MouseEvent )
{
	...
	else if ( bHandleLeftMouseButton )
	{
		...
		else if(MouseDragType == DRAG_TIME)
		{
			...
			if(!MouseEvent.IsControlDown())
			{
				...
				EditorTimelineController.Pin()->Snap(Time, SnapMargin, { FName("MontageSection") });
			}
...
FReply FNeTimeSliderController::OnMouseMove( SWidget& WidgetOwner, const FGeometry& MyGeometry, const FPointerEvent& MouseEvent )
{
	...
	else if (bHandleLeftMouseButton)
	{
		...
		if ( MouseDragType == DRAG_NONE )
		{
			if ( DistanceDragged > FSlateApplication::Get().GetDragTriggerDistance() )
			{
				//UAnimMontage* AnimMontage = Cast<UAnimMontage>(EditorTimelineController.Pin()->GetAnimSequenceBase());
...
void FNeTimeSliderController::CommitScrubPosition( FFrameTime NewValue, bool bIsScrubbing )
{
	...
	TimeSliderArgs.OnScrubPositionChanged.ExecuteIfBound( NewValue, bIsScrubbing, true);
}

FReply FNeTimeSliderController::OnMouseWheel( SWidget& WidgetOwner, const FGeometry& MyGeometry, const FPointerEvent& MouseEvent )
{
...
FCursorReply FNeTimeSliderController::OnCursorQuery( TSharedRef<const SWidget> WidgetOwner, const FGeometry& MyGeometry, const FPointerEvent& CursorEvent ) const
{
...
int32 FNeTimeSliderController::OnPaintViewArea( const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, bool bEnabled, const FPaintViewAreaArgs& Args ) const
{
	const ESlateDrawEffect DrawEffects = bEnabled ? ESlateDrawEffect::None : ESlateDrawEffect::DisabledEffect;
	...
	{
		// Draw a line for the scrub position
		const float LinePos = RangeToScreen.InputToLocalX(TimelineMode.Pin()->GetScrubTime());
...
FFrameTime FNeTimeSliderController::GetFrameTimeFromMouse(const FGeometry& Geometry, FVector2D ScreenSpacePosition) const
{
...
TSharedRef<SWidget> FNeTimeSliderController::OpenSetPlaybackRangeMenu(FFrameNumber FrameNumber)
{
	...
	MenuBuilder.BeginSection("AnimSelectionRangeMenu", FText::Format(LOCTEXT("SelectionRangeTextFormat", "Selection Range ({0}):"), CurrentTimeText));
	{
		MenuBuilder.AddMenuEntry(
			...
			FSlateIcon(),
			FUIAction(
				FExecuteAction::CreateLambda([=, this]{ SetSelectionRangeStart(FrameNumber); }),
		...
		MenuBuilder.AddMenuEntry(
			...
			FUIAction(
				...
				FCanExecuteAction::CreateLambda([=, this]{ return SelectionRange.IsEmpty() || FrameNumber >= UE::MovieScene::DiscreteInclusiveLower(SelectionRange); })
			)
		);
...
void FNeTimeSliderController::ClampViewRange(double& NewRangeMin, double& NewRangeMax)
{
...
void FNeTimeSliderController::SetViewRange( double NewRangeMin, double NewRangeMax, EViewRangeInterpolation Interpolation )
{
...
void FNeTimeSliderController::SetClampRange( double NewRangeMin, double NewRangeMax )
{
...
void FNeTimeSliderController::SetPlayRange( FFrameNumber RangeStart, int32 RangeDuration )
{
...
bool FNeTimeSliderController::ZoomByDelta( float InDelta, float MousePositionFraction )
{
...
void FNeTimeSliderController::PanByDelta( float InDelta )
{
	TRange<double> LocalViewRange = TimeSliderArgs.ViewRange.Get().GetAnimationTarget();
...
bool FNeTimeSliderController::HitTestRangeStart(const FScrubRangeToScreen& RangeToScreen, const TRange<double>& Range, float HitPixel) const
{
...
bool FNeTimeSliderController::HitTestRangeEnd(const FScrubRangeToScreen& RangeToScreen, const TRange<double>& Range, float HitPixel) const
{
...
bool FNeTimeSliderController::HitTestTime(const FScrubRangeToScreen& RangeToScreen, double Time, float HitPixel) const
{
...
void FNeTimeSliderController::SetPlaybackRangeStart(FFrameNumber NewStart)
{
	TRange<FFrameNumber> PlaybackRange = TimeSliderArgs.PlaybackRange.Get();
...
void FNeTimeSliderController::SetPlaybackRangeEnd(FFrameNumber NewEnd)
{
	TRange<FFrameNumber> PlaybackRange = TimeSliderArgs.PlaybackRange.Get();
...
void FNeTimeSliderController::SetSelectionRangeStart(FFrameNumber NewStart)
{
	TRange<FFrameNumber> SelectionRange = TimeSliderArgs.SelectionRange.Get();
...
void FNeTimeSliderController::SetSelectionRangeEnd(FFrameNumber NewEnd)
{
	TRange<FFrameNumber> SelectionRange = TimeSliderArgs.SelectionRange.Get();
...
int32 FNeTimeSliderController::HitTestTimes(const FScrubRangeToScreen& RangeToScreen, float HitPixel) const
{
	/*const TArray<double>& Times = EditorTimelineController.Pin()->GetEditableTimes();
	int32 NumTimes = Times.Num();
	for(int32 TimeIndex = 0; TimeIndex < NumTimes; ++TimeIndex)
	{
		const double Time = EditorTimelineController.Pin()->GetEditableTimes()[TimeIndex];
